...
async def callback(event):
    if event.sender_id == admin_user_id and event.data == b"langfa":
        ...
        \n`/addbio`: این دستور برای اضافه کردن یک بیوگرافی یا توضیح کوتاه به پروفایل شما می‌تواند استفاده شود، که می‌تواند به دیگران در یادگیری بیشتر درباره شما کمک کند.
        ...
        \n`/tr`: این دستور برای ترجمه متن از یک زبان به زبان دیگر با استفاده از یک سرویس ترجمه آنلاین استفاده می‌شود.
        ...
        \n`/expire`:**برای مشاهده زمان باقی مانده از اشتراک شما.**
        \n`/restore` [number] : برای خرید دوباره اشتراک میباشد.
        ...
        \n`/azan` or `/اذان` [city]:اوقات شرعی شهر مورد نظر را نشان میدهد.
